...
public:
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns the singleton instance of the Dereferee memory manager, or null
	 * if it does not exist. Used in the overridden operator delete to make
	 * sure that memory freed after the manager is destroyed (this may happen
	 * for reasons possibly related to initialization sequences) do not
	 * recreate the manager and just ignore the request (the operating system
	 * will clean up the mess).
	 *
	 * @returns the single Dereferee::manager instance
	 */
	static manager* existing_instance();
