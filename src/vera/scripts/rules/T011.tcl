...
proc acceptPairs {} {
    ...
    while {$index != $end} {
        ...
        if {$tokenValue == "\{"} {
            ...
            if {$index == $end} {
                report $file $leftParenLine "The opening curly brace is not closed."
                return
            }
            ...
            if {($leftParenLine != $rightParenLine) && ($leftParenColumn != $rightParenColumn)} {
                ...
                if {[string index $leftLine end] != "\\" && [string index $rightLine end] != "\\"} {
                    report $file $rightParenLine "The closing curly brace not in the same line or column as the matching opening curly brace."
                }
            }
        } else {
            return
...
foreach file [getSourceFileNames] {
    ...
    acceptPairs
    if {$index != $end} {
        report $file [lindex [lindex $parens $index] 1] "Excessive closing curly brace?"
    }
}
